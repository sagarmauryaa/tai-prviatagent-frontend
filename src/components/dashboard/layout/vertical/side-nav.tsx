...
type NavColor = "blend_in" | "discrete" | "evident";
...
export function SideNav({ color = "evident", items = [], onClose }: SideNavProps) {
	const pathname = usePathname();

	const { mode: colorScheme = "light" } = useColorScheme();
			...
			<Stack spacing={2} sx={{ p: 2 }}>
				<div>
					<Box component={RouterLink} href={paths.home} sx={{ display: "inline-flex" }}>
						<Logo color={"dark"} height={46} width={122} />
					</Box>
				</div>
			</Stack>
			<Box
...
function renderNavGroups({ items = [], pathname }: { items: NavItem[]; pathname: string }) {
...
function renderNavItems({
	...
	const children = items.reduce<React.ReactNode[]>((acc, curr) => {
		...
		const forceOpen = childItems
			? childItems.some((childItem: NavItemType) => childItem.href && pathname.startsWith(childItem.href))
			: false;

		acc.push(
			<NavItem key={key} {...(item as NavItemProps)} depth={depth} forceOpen={forceOpen} pathname={pathname}>
